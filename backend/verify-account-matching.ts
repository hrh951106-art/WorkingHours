...
async function main() {
  ...
  console.log('线体工时配置:');
  console.log(`  账户层级配置: ${accountLevels}`); // [0,1,2]
  console.log(`  对应层级: level${accountLevels.map(l => l + 1).join(', level')}`); // level1, level2, level3
  ...
  const testCases = [
    {
      ...
      hierarchyValues: [
        ...
        { level: 3, name: '���线', mappingType: 'ORG', selectedValue: { name: 'W1总装车间L1产线' } },
      ],
      expected: true,
    },
    {
      name: '场景2：账户只有工厂、车间（缺少产线）',
      hierarchyValues: [
        { level: 1, name: '工厂', mappingType: 'ORG', selectedValue: { name: '大华富阳工厂' } },
        { level: 2, name: '车间', mappingType: 'ORG', selectedValue: { name: 'W1总装车间' } },
      ],
    ...
    {
      name: '场景3：账户有工厂、车间、产线、产品（超出配置）',
      hierarchyValues: [
        { level: 1, name: '工厂', mappingType: 'ORG', selectedValue: { name: '大华富阳工厂' } },
        { level: 2, name: '车间', mappingType: 'ORG', selectedValue: { name: 'W1总装车间' } },
        { level: 3, name: '产线', mappingType: 'ORG', selectedValue: { name: 'W1总装车间L1产线' } },
        { level: 4, name: '产品', mappingType: 'FIELD_A01', selectedValue: { name: '大桶' } },
      ],
      expected: false,
    },
    {
      name: '场景4：账户有工厂、车间、产线、工序（超出配置）',
      hierarchyValues: [
        { level: 1, name: '工厂', mappingType: 'ORG', selectedValue: { name: '大华富阳工厂' } },
        { level: 2, name: '车间', mappingType: 'ORG', selectedValue: { name: 'W1总装车间' } },
        { level: 3, name: '产线', mappingType: 'ORG', selectedValue: { name: 'W1总装车间L1产线' } },
  ...
  for (const testCase of testCases) {
    ...
    // 找出账户中有值的所有层级（组织层级：mappingType为ORG或ORG_TYPE）
    const filledOrgLevels = hierarchyValues
      .filter((hv: any) =>
        (hv.mappingType === 'ORG_TYPE' || hv.mappingType === 'ORG') && hv.selectedValue
      )
      .map((hv: any) => hv.level);
    ...
    for (const filledLevel of filledOrgLevels) {
    ...
    console.log(`  账户层级: ${hierarchyValues.map(hv => `level${hv.level}(${hv.name})`).join(', ')}`);
    console.log(`  配置的层级: ${configLevels.join(', ')}`);
    console.log(`  有值的层级: ${filledOrgLevels.join(', ')}`);
    console.log(`  检查1（配置的层级都有值）: ${check1Passed ? '✅' : '❌'}`);
    console.log(`  检查2（不能有配置之外的层级）: ${check2Passed ? '✅' : '❌'}`);
  ...
  for (const pp of punchPairs) {
    ...
    console.log(`PunchPair ID: ${pp.id}, shiftId: ${pp.shiftId}`);
