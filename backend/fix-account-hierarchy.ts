...
async function main() {
  console.log('=== 修复账户22和27的hierarchyValues ===\n');
  ...
  console.log(`大华工厂: ${company ? `ID=${company.id}, Code=${company.code}` : '未找到'}`);
  ...
  // 查找"焊接"（从工作信息字段）
  // const weldingProcess = await prisma.employeeWorkInfo.findFirst({
  //   where: {
  //     customFields: {
  //       path: ['$'],
  //       array_contains: { fieldName: '工序', value: '焊接' }
  //     }
  //   }
  // });
  ...
  // 或者从字段数据源查找
  console.log('');
  console.log('步骤2：构建hierarchyValues\n');
