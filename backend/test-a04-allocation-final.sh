...
echo "结果:"
echo "$RESULTS" | python3 -c "import sys, json; data=json.load(sys.stdin); items=data.get('data', {}).get('items', []); print(f\"找到 {len(items)} 条分摊结果\"); [print(f\"- {item.get('sourceEmployeeNo')}: {item.get('sourceHours')}h -> {item.get('targetName')}: {item.get('allocatedHours')}h\") for item in items[:10]]" 2>/dev/null || echo "$RESULTS"
