...
async function main() {
  ...
  try {
    ...
    workHourResults.forEach(result => {
      const key = result.employee?.employeeNo || 'Unknown';
    ...
    employeeGroups.forEach((results, employeeNo) => {
      const name = results[0].employee?.name || 'Unknown';
      const totalHours = results.reduce((sum, r) => sum + (r.actualHours || 0), 0);
      const totalAmount = results.reduce((sum, r) => sum + (r.amount || 0), 0);
      console.log(`员工 ${employeeNo} (${name}):`);
    ...
    workHourResults.slice(0, 10).forEach((result, index) => {
      ...
      console.log(`  员工: ${result.employee?.employeeNo || 'N/A'} (${result.employee?.name || 'N/A'})`);
      console.log(`  工作日期: ${result.workDate ? result.workDate.toISOString().split('T')[0] : 'N/A'}`);
      console.log(`  出勤代码: ${result.attendanceCode || 'N/A'}`);
      console.log(`  实际工时: ${result.actualHours || 0} 小时`);
      console.log(`  标准工时: ${result.standardHours || 0} 小时`);
