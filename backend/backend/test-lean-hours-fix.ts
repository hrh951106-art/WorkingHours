...
async function main() {
  ...
  const attendanceCodeService = new AttendanceCodeService(prisma);
  ...
  // 查询出勤代码配置
  const codes = await prisma.calculationAttendanceCodeConfig.findMany({
    where: {
      code: {
        in: ['AC_001', 'AC_003'],
      },
  ...
  console.log('\n最终结果:');
  results.forEach(r => {
    console.log(`- ${r.name} (${r.code}): ${r.workHour.toFixed(2)} 小时`);
  });
}
